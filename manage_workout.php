...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['update'])) {
        $plan = mysqli_real_escape_string($conn, $_POST['Plan_type_id']);
        $date = mysqli_real_escape_string($conn, $_POST['Workout_date']);
        $notes = mysqli_real_escape_string($conn, $_POST['Notes']);

        $update = "UPDATE member_schedule 
                   SET Plan_type_id='$plan', Workout_date='$date', Notes='$notes' 
...
<h1>Manage Workout for <?php echo htmlspecialchars($workout['Mem_name']); ?></h1>
    ...
    <select name="Plan_type_id" required>
        <?php while ($p = mysqli_fetch_assoc($plans)): ?>
            <option value="<?php echo $p['Plan_type_id']; ?>" 
                ...
                <?php echo htmlspecialchars($p['Workout_type']); ?>
            </option>
        <?php endwhile; ?>
    </select>

    <label>Date:</label>
    <input type="date" name="Workout_date" value="<?php echo htmlspecialchars($workout['Workout_date']); ?>" required>

    <label>Description / Notes:</label>
    <textarea name="Notes" required><?php echo htmlspecialchars($workout['Notes']); ?></textarea>
